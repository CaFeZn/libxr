 ...
 *
 * BLOCK 模式保持同步阻塞式传输；POLLING / CALLBACK / NONE 额外提供第一版 DMA
 * 后台路径，目前覆盖 Write / Read / MemRead，MemWrite 仍保持阻塞路径。对 10-bit /
 * 扩展 flags 的 blocking 手工 phase 路径，地址阶段会显式等待 `ADDRHIT`，失败时按
 * 适配层策略强制 cleanup 并按原始 HPM 状态触发恢复链路。
 * BLOCK mode stays on synchronous blocking transfers; POLLING / CALLBACK / NONE
 * additionally use a first-pass DMA backed asynchronous path for Write / Read /
 * MemRead, while MemWrite remains blocking in this round. For the blocking manual
 * phase path used by 10-bit / extended-flag transfers, the address phase explicitly
 * waits for `ADDRHIT`; failures force adapter-level cleanup and preserve the original
 * HPM status for recovery handling.
 *
 ...
 *
 * 默认 ISR wrapper 会为当前 HPM5301 模板和当前 SoC 暴露的 I2C 实例提供中断分发；
 * 若项目层后续要自行接管同一 IRQ，需要单独调整 ownership 方案。
 * The default ISR wrapper provides interrupt dispatch for the current HPM5301
 * template and the I2C instances exposed by the current SoC. If project code needs
 * to own the same IRQ later, the ownership scheme should be revised in a separate
 * change.
 */
