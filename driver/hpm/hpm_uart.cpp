...
{
  auto* uart = CONTAINER_OF(&port, HPMUART, _write_port);
...
{
  auto* uart = CONTAINER_OF(&port, HPMUART, _read_port);
...
void HPMUART::HandleTxDmaComplete(ErrorCode result)
{
  ...
  {
    HPMUARTDetail::FailAndClearPendingWrites(*write_port_, dma_buff_tx_, result, true);
    return;
  }
