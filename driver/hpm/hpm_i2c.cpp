...
static void FlushDCacheIfNeeded(const void* addr, uint32_t size)
{
  ...
  {
    l1c_dc_flush(reinterpret_cast<uint32_t>(addr), size);
  }
#else
  UNUSED(addr);
  UNUSED(size);
...
static void InvalidateDCacheIfNeeded(const void* addr, uint32_t size)
{
  ...
  if (addr != nullptr && size > 0U && l1c_dc_is_enabled())
  {
    l1c_dc_invalidate(reinterpret_cast<uint32_t>(addr), size);
  }
#else
  UNUSED(addr);
  UNUSED(size);
...
ErrorCode HPMI2C::StartAsyncReadDma(void* dst, uint32_t size)
{
  ...
  InvalidateDCacheIfNeeded(dst, size);
                                  ...
                                  WriteOperation& op)
{
  ...
  ans = PrepareAsyncTransfer(slave_addr, 0U, static_cast<uint32_t>(write_data.size_));
  if (ans != ErrorCode::OK)
  {
    async_busy_.store(0U, std::memory_order_release);
    async_ctx_ = {};
                                 ...
                                 ReadOperation& op)
{
  ...
  ans = PrepareAsyncTransfer(slave_addr, kI2CFlagRead,
                                    ...
                                    MemAddrLength mem_addr_size)
{
  ...
  ans = PrepareAsyncTransfer(slave_addr, kI2CFlagRead,
                             static_cast<uint32_t>(read_data.size_), true, false);
  if (ans != ErrorCode::OK)
  {
    async_ctx_.should_recover = true;
    ReleaseAsyncBus();
...
void HPMI2C::HandleAsyncInterrupt(bool in_isr)
{
  ...
  if ((status & I2C_STATUS_CMPL_MASK) != 0U)
  {
    ...
    else if (i2c_get_data_count(i2c_) != 0U)
    {
